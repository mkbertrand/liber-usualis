...
def kalendar(year: int) -> Kalendar:
	...
	noprimarium = roletags | {'psalmi-graduales', 'psalmi-poenitentiales', 'officium-defunctorum', 'antiphona-bmv'}
					...
					if job.rule['response'] == 'combina':
						...
						if job.rule['response'] == 'dele':
							...
							if job.rule['response'] <= kal[day][target]:
								continue
							response = job.rule['response']
							if 'duplicate' in job.rule['response']:
								kal[day].append(copy.deepcopy(kal[day][target]))
								target += 1
								response = job.rule['response'] - {'duplicate'}
							if response & roletags:
								kal[day][target] -= roletags
							kal[day][target] |= response
							queue.append(job)
